...
/// Kernel names of every RPN kernel that implements a *lazy-sensitive* SQL
/// construct: a signature whose Go/MySQL row evaluator may skip one or more
/// children (`IF`, `IFNULL`, `COALESCE`, `CASE`, `AND`, `OR`, `XOR`, `ELT`,
/// `FIELD`, `INTERVAL`, `GREATEST`/`LEAST`, and the `AddTime*Null` forms that
/// never look at their arguments).
///
/// This list is deliberately *not* restricted to the families that already
/// carry a `lazy_fn_ptr`. The embedder uses it through
...
/// enters. A family therefore only leaves this list when its eager kernel is
/// deleted, not when it is merely not lazy yet.
///
...
pub(crate) const LAZY_SENSITIVE_KERNELS: &[&str] = &[
    ...
    "logical_or",
    "logical_xor",
    // Tier 2 families that are still eagerly dispatched today.
    "elt",
    "field",
    "field_bytes",
    "interval_int",
    ...
    "greatest_string",
    "greatest_datetime",
    "greatest_cmp_string_as_date",
    "greatest_cmp_string_as_time",
    "greatest_duration",
    "least_int",
    "least_real",
    "least_decimal",
    "least_string",
    "least_datetime",
    "least_cmp_string_as_date",
    "least_cmp_string_as_time",
    "least_duration",
