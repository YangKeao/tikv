...
fn map_field_string_sig(ret_field_type: &FieldType) -> Result<RpnFnMeta> {
    Ok(match_template_collator! {
        TT, match ret_field_type.as_accessor().collation().map_err(tidb_query_datatype::codec::Error::from)? {
            Collation::TT => field_bytes_fn_meta::<TT>().with_lazy(lazy_field_bytes::<TT>)
        }
    })
}

/// Kernel names of the RPN kernels that implement a *lazy-sensitive* SQL
/// construct: a signature whose Go/MySQL row evaluator may skip one or more
/// children (`IF`, `IFNULL`, `COALESCE`, `CASE`, `AND`, `OR`, `XOR`, and the
/// Tier 3 `AddTime*Null` forms that never look at their arguments).
///
/// The embedder uses it through
/// [`standalone::PreparedExpression::eager_lazy_risk`] to detect a program
/// that mixes lazily dispatched nodes with an eager node of a different
/// lazy-sensitive family; such a program would enter a branch MySQL never
/// enters. A family is removed from this list only once *every* signature it
/// dispatches carries a `lazy_fn_ptr`, so the dispatcher can no longer produce
/// an eager node of that family and there is nothing left for the risk scan to
/// report. `lazy_sensitive_tests` enforces that rule against the real
/// dispatcher.
///
/// The Tier 1 control/logical names are kept although they are fully lazy
/// today: the risk scan checks `lazy_fn_ptr` first, so a fully lazy name is
/// inert, and dropping it would only hide a future eager regression. The
/// Tier 2 families (`elt`, `field*`, `interval*`, `greatest*`, `least*`) left
/// the list when their lazy kernels were registered.
///
/// The names are the `RpnFnMeta::name` values generated by `#[rpn_fn]` (the
/// kernel function identifier), not the wire `ScalarFuncSig` names, because
/// `RpnExpressionNode::FnCall` carries the meta and not the signature.
pub(crate) const LAZY_SENSITIVE_KERNELS: &[&str] = &[
    // Tier 1 control/logical kernels; all registered with `with_lazy` below.
    "if_condition",
    "if_condition_bytes",
    "if_condition_json",
    "if_null",
...
fn map_expr_node_to_rpn_func(expr: &Expr) -> Result<RpnFnMeta> {
    ...
    Ok(match value {
        ...
        ScalarFuncSig::GreatestInt => greatest_int_fn_meta().with_lazy(lazy_greatest::<Int>),
        ScalarFuncSig::GreatestDecimal => {
            greatest_decimal_fn_meta().with_lazy(lazy_greatest::<Decimal>)
        }
        ScalarFuncSig::GreatestString => greatest_string_fn_meta().with_lazy(lazy_greatest_bytes),
        ScalarFuncSig::GreatestReal => greatest_real_fn_meta().with_lazy(lazy_greatest::<Real>),
        ScalarFuncSig::GreatestTime |
        ScalarFuncSig::GreatestDate => greatest_datetime_fn_meta().with_lazy(lazy_greatest::<DateTime>),
        ScalarFuncSig::GreatestCmpStringAsDate => {
            greatest_cmp_string_as_date_fn_meta().with_lazy(lazy_greatest_cmp_string_as_date)
        }
        ScalarFuncSig::GreatestCmpStringAsTime => {
            greatest_cmp_string_as_time_fn_meta().with_lazy(lazy_greatest_cmp_string_as_time)
        }
        ScalarFuncSig::GreatestDuration => greatest_duration_fn_meta().with_lazy(lazy_greatest::<Duration>),
        ScalarFuncSig::LeastInt => least_int_fn_meta().with_lazy(lazy_least::<Int>),
        ScalarFuncSig::IntervalInt => interval_int_fn_meta().with_lazy(lazy_interval_int),
        ScalarFuncSig::LeastDecimal => least_decimal_fn_meta().with_lazy(lazy_least::<Decimal>),
        ScalarFuncSig::LeastString => least_string_fn_meta().with_lazy(lazy_least_bytes),
        ScalarFuncSig::LeastReal => least_real_fn_meta().with_lazy(lazy_least::<Real>),
        ScalarFuncSig::LeastTime |
        ScalarFuncSig::LeastDate=> least_datetime_fn_meta().with_lazy(lazy_least::<DateTime>),
        ScalarFuncSig::LeastCmpStringAsDate => {
            least_cmp_string_as_date_fn_meta().with_lazy(lazy_least_cmp_string_as_date)
        }
        ScalarFuncSig::LeastCmpStringAsTime=> {
            least_cmp_string_as_time_fn_meta().with_lazy(lazy_least_cmp_string_as_time)
        }
        ScalarFuncSig::LeastDuration => least_duration_fn_meta().with_lazy(lazy_least::<Duration>),
        ScalarFuncSig::IntervalReal => interval_real_fn_meta().with_lazy(lazy_interval_real),
        ...
        ScalarFuncSig::FieldInt => field_fn_meta::<Int>().with_lazy(lazy_field::<Int>),
        ScalarFuncSig::FieldReal => field_fn_meta::<Real>().with_lazy(lazy_field::<Real>),
        ScalarFuncSig::FieldString => map_field_string_sig(ft)?,
        ScalarFuncSig::Elt => elt_fn_meta().with_lazy(lazy_elt),
