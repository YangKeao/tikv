...
| 9 | `Quarter` over an implicit temporal cast containing a zero month | returns NULL for `QUARTER('2008-00-01')` in a mixed projection | returns 0 under the source fixture's `IgnoreZeroInDate` semantics | avoided | TiDB Rust contracts only the exact mixed nine-column QUARTER statement; valid-date QUARTER rows remain TiKV-required, and an independent engine receipt covers other calendar-component signatures plus a positive engine-row delta |
| 10 | `PeriodAdd` at the signed-integer maximum | `PERIOD_ADD(9223372036854775807, 1)` returns 27201459512 | the Go uint64 helper plus int64 conversion wraps to `-9223372036854775808` | avoided | only this exact expression/SELECT is contracted; ordinary PERIOD_ADD remains TiKV-required and is covered by an independent value plus engine-row receipt |
